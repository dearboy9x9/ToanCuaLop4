...
# Cập nhật 100% theo ảnh chụp Book Map của ông chủ
ENGLISH_BOOK_MAP = {
...
# --- 3. HÀM DỮ LIỆU & HIỂN THỊ KHOA HỌC (FIX LỖI KEYERROR) ---
def load_data():
...
if mode == "🚀 Làm bài mới":
    ...
    if st.button("📝 RA ĐỀ CHI TIẾT"):
        ...
        with st.spinner("AI đang soạn đề bài khoa học..."):
            if "Toán" in mon_hoc:
        ...
        with st.form("exam_form"):
            ...
            if st.form_submit_button("✅ NỘP BÀI & CHẤM CHI TIẾT"):
                with st.spinner("Đang soi xét từng câu..."):
                    ...
                    prompt_grading = f"""Bạn là giáo viên nghiêm khắc. Chấm điểm thang 10. Chấm chi tiết từng câu từ 1-9.
                    - Key: {st.session_state['raw_ans']}
                    - HS: TN {ans_hs}, {status_tl}.
                    Trả về: 1. KẾT QUẢ TỪNG CÂU (Đúng/Sai - Giải thích) | 2. DIEM: [Số] | 3. YEU: [Kiến thức yếu]"""
                    ...
                    try:
                        ...
                        new_row = {"Time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Mon": mon_hoc, "Diem": d, "Coins": coins, "Yeu": "Xem giải thích", "Phut": 0}

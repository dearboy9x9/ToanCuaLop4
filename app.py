...
def process_text_to_html(text, title, color_hex):
    ...
    text = re.sub(r'(Câu \d+[:\.])', r'<br><b style="color: #d35400; font-size: 1.2em; display: inline-block; margin-top: 15px;">\2</b>', text)
...
# --- 4. HÀM ÂM THANH TỐI ƯU CHO IPAD (V83 FIX) ---
...
def play_pro_audio(text, speed="Normal"):
    # Fix cho iPad: Chuyển đổi sang BytesIO để trình duyệt nhận diện file chuẩn hơn
    ...
    # Ép kiểu dữ liệu để Safari trên iPad không thể từ chối
    st.audio(io.BytesIO(audio_bytes), format='audio/mp3')

# --- 5. GIAO DIỆN CHÍNH ---
st.set_page_config(page_title="Academy V83 - iPad Pro", layout="wide")
if 'html_p1' not in st.session_state:
    st.session_state.update({'html_p1':"", 'html_p2':"", 'raw_ans':"", 'ket_qua':"", 'start_time': None, 'listening_text': ""})

with st.sidebar:
    st.title("🛡️ SUPREME V83")
...
# --- 6. LOGIC RA ĐỀ VÀ HIỂN THỊ ---
if mode == "🚀 Làm bài mới":
    if st.button("📝 RA ĐỀ CHUẨN (IPAD READY)"):
    ...
    if st.session_state['html_p1']:
        if st.session_state['listening_text']:
            with st.expander("🎧 BẤM ĐỂ NGHE (ĐÃ TỐI ƯU CHO IPAD)"): 
